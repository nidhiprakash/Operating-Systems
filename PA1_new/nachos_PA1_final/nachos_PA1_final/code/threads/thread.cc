...
Thread::Fork(Process *p, VoidFunctionPtr func, void *arg)
{
    ...
    p->pScheduler->ReadyToRun(this);	// ReadyToRun assumes that interrupts
...
Thread::Begin ()
{
    ...
    std::cout << "Beginning thread: " << name << endl;
...
Thread::Finish ()
{
    ...
    std::cout << "In thread finish. Finishing thread: " << name << endl;
    isFinished = true;
...
Thread::Yield ()
{
    ...
    std::cout << "Yielding thread: " << name << endl;
    
    nextThread = kernel->currentProcess->pScheduler->FindNextToRun();
    if (nextThread != NULL) {
	kernel->currentProcess->pScheduler->ReadyToRun(this);
	kernel->currentProcess->pScheduler->Run(nextThread, FALSE);
    } else {
        bool allDone = true;

        ListIterator<Thread*> *li = new ListIterator<Thread*>(kernel->currentProcess->cThreads);
        while (!li->IsDone()) {
            if(!li->Item()->isFinished)
                allDone = false;
            li->Next();
        }
...
Thread::Sleep (bool finishing)
{
    ...
    std::cout << "Sleeping thread: " << name << endl;

    status = BLOCKED;
    while ((nextThread = kernel->currentProcess->pScheduler->FindNextToRun()) == NULL)
	    kernel->interrupt->Idle();	// no one to run, wait for an interrupt
    
    // returns when it's time for us to run
    kernel->currentProcess->pScheduler->Run(nextThread, finishing); 
}
...
Thread::SelfTest()
{
    ...
    p->currentThread->Fork(p, (VoidFunctionPtr) SimpleThread, (void *) 1);
    kernel->currentProcess->Yield();
    SimpleThread(0);
}
